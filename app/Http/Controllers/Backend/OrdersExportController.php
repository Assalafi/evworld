...
class OrdersExportController extends Controller
{
    ...
    public function OrdersExcelExport(Request $request){
		...
		if(($start_date != '') && ($end_date != '')){
			...
			if($status == 0){
				...
				$datalist = DB::table('order_masters')
					...
					->join('order_status', 'order_masters.order_status_id', '=', 'order_status.id')
					->select('order_masters.*', 'users.name', 'payment_method.method_name', 'payment_status.pstatus_name', 'order_status.ostatus_name')
					->where('order_masters.order_status_id', '=', $status)
					->orderBy('order_masters.created_at','desc')
					->get();
			}
		}
		...
		$spreadsheet -> getActiveSheet()				
					...
					->SetCellValue('E4', __('Amount').'('.$gtext['currency_icon'].')')							
					->SetCellValue('F4', __('Qty'))
					->SetCellValue('G4', __('Payment Method'))
					->SetCellValue('H4', __('Payment Status'))
					->SetCellValue('I4', __('Order Status'));
		...
		foreach($datalist as $row){
			...
			$spreadsheet->getActiveSheet()
						...
						->SetCellValue('E'.$j, $total_amount)																
						->SetCellValue('F'.$j, $row->total_qty)
						->SetCellValue('G'.$j, $row->method_name)
						->SetCellValue('H'.$j, $row->pstatus_name)
						->SetCellValue('I'.$j, $row->ostatus_name);
    ...
    public function OrdersCSVExport(Request $request){
		...
		$spreadsheet->getActiveSheet() -> SetCellValue('E2', __('Orders'));	
