            ...
            <div class="row">
                <div class="col-12 col-sm-12 col-md-12 col-lg-5 col-xl-5 col-xxl-5 mb25">
                    @if (count($pro_images) > 0)
                        <div id="product_big" class="single-product-slider owl-carousel caro-single-product">
                            @foreach ($pro_images as $row)
                                <div class="item">
                                    <img src="{{ asset('media/' . $row->large_image) }}" />
                                </div>
                            @endforeach
                        </div>
                    ...
                    @else
                        <div id="product_big" class="single-product-slider owl-carousel caro-single-product">
                            <div class="item">
                                <img src="{{ asset('media/' . $data->f_thumbnail) }}" />
                            </div>
                        </div>
                        <div id="product_thumbs" class="thumbnail-card owl-carousel">
                            <div class="item">
                    ...
                    <div class="pr_details">
                        ...
                        @if ($data->availability != '')
                            <div class="pr_extra">
                                ...
                                <span style="color: {{ $data->availability == 'Sold Out' ? '#ff0000' : $data->labelcolor }};">
                                    {{ $data->availability }}
                                </span>
                            </div>
                        @endif
                        ...
                        <div class="product_price">
                            @if ($data->sale_price != '')
                                @if ($gtext['currency_position'] == 'left')
                                    <div class="item-price">{{ $gtext['currency_icon'] }}{{ number_format($data->sale_price, 2) }}</div>
                                @else
                                    <div class="item-price">{{ number_format($data->sale_price, 2) }}{{ $gtext['currency_icon'] }}</div>
                                @endif
                            @endif
                            @if ($data->old_price != '')
                                @if ($gtext['currency_position'] == 'left')
                                    <div class="old-item-price">{{ $gtext['currency_icon'] }}{{ number_format($data->old_price, 2) }}</div>
                                @else
                                    <div class="old-item-price">{{ number_format($data->old_price, 2) }}{{ $gtext['currency_icon'] }}</div>
                                @endif
                            @endif
                        </div>
                                ...
                                <ul class="widget-color">
                                    @foreach (explode(',', $data->variation_color) as $color)
                                        ...
                                        <li id="color_{{ $color_array[0] }}" class="tp_color">
                                            <a data-id="{{ $color_array[0] }}" class="selectcolor"
                                                href="javascript:void(0);" title="{{ $color_array[0] }}"><span
                                                    style="background:{{ $color_array[1] }};"></span></a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                                ...
                                <ul class="widget-size">
                                    @foreach (explode(',', $data->variation_size) as $size)
                                        <li id="size_{{ $size }}" class="tp_size">
                                            <a data-id="{{ $size }}" class="selectsize"
                        ...
                        <div class="pr_buy_cart">
                            <a class="btn theme-btn cart product_buy_now"
                                href="javascript:void(0);"
                                onclick="showContactOptions(this)">
                                {{ __('Contact to Place an Order') }}
                            </a>
                        </div>

                        <script>
                            function showContactOptions(button) {
                                if (button.getAttribute('data-expanded') !== 'true') {
                                    const phoneNumber = '{{ $gtext['phone_footer'] }}';
                                    const intlNumber = '+234' + phoneNumber.substring(1);
                                    
                                    const callSpan = document.createElement('span');
                                    callSpan.style.cursor = 'pointer';
                                    callSpan.onclick = function(e) {
                                        e.stopPropagation();
                                        window.location.href = 'tel:' + intlNumber;
                                    };
                                    callSpan.textContent = 'Call ' + intlNumber;

                                    const whatsappSpan = document.createElement('span');
                                    whatsappSpan.style.cursor = 'pointer';
                                    whatsappSpan.style.color = '#25D366';
                                    whatsappSpan.onclick = function(e) {
                                        e.stopPropagation();
                                        window.open('https://wa.me/' + intlNumber + '/?text=Hello, I am interested in {{ urlencode($data->title) }}', '_blank');
                                    };
                                    whatsappSpan.textContent = 'Via WhatsApp';

                                    button.innerHTML = '';
                                    button.appendChild(callSpan);
                                    button.appendChild(document.createTextNode(' or '));
                                    button.appendChild(whatsappSpan);
                                    button.setAttribute('data-expanded', 'true');
                                }
                            }
                        </script>
                        {{-- <div id="variation_required">
                            <p class="red">{{ __('Please select required field.') }}</p>
                        </div>
                        <div id="quantity_required">
                            <p class="red">{{ __('Please enter quantity.') }}</p>
                        </div>
                        <div id="stockqty_required">
                            <p class="red">{{ __('The value must be less than or equal to') }}
                                {{ $data->is_stock == 1 ? $data->stock_qty : '' }}</p>
                        </div>
                        <div id="stockout_required">
                            <p class="red">{{ __('This product out of stock.') }}</p>
                        </div> --}}
                ...
                <div class="col-lg-12">
                    <div class="pr-description-review">
                        <div class="desc-review-nav nav">
                            ...
                            <a href="#des_reviews" data-bs-toggle="tab">{{ __('Reviews') }}
                                ({{ $data->TotalReview }})</a>
                        </div>
                                ...
                                <div class="review-content">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6">
                                            ...
                                            <p class="theme-color">Please <a
                                                    href="{{ route('frontend.login') }}">login</a> to write review!</p>
                                            <div class="form-product-review">
                                                <form class="form" method="POST"
                                                    action="{{ route('frontend.saveReviews') }}">
                                                    @csrf
                                                    ...
                                                    @if (isset(Auth::user()->name))
                                                        ...
                                                        <input name="item_id" type="hidden"
                                                            value="{{ $data->id }}" />
                                                        <button type="submit"
                                                            class="btn theme-btn">{{ __('Submit Review') }}</button>
                                                    @else
                                                        <div class="mb-3">
        ...
        <div class="container">
            <div class="row">
                <div class="section-heading">
                    ...
                    <a class="btn theme-btn seeall-btn"
                        href="{{ route('frontend.product-category', [$data->cat_id, $data->cat_slug]) }}">{{ __('See all') }}</a>
                </div>
            </div>
                                ...
                                <div class="item-image">
                                    @if ($row->labelname != '')
                                        <ul class="labels-list">
                                            <li><span class="tplabel"
                                                    style="background:{{ $row->labelcolor }};">{{ $row->labelname }}</span>
                                            </li>
                                        </ul>
                                    @endif
                                    <ul class="product-action">
                                        ...
                                        @else
                                            <li><a class="addtocart" data-id="{{ $row->id }}"
                                                    href="javascript:void(0);"><i class="bi bi-cart"></i></a></li>
                                        @endif
                                        <li><a href="{{ route('frontend.product', [$row->id, $row->slug]) }}"><i
                                                    class="bi bi-zoom-in"></i></a></li>
                                    ...
                                    <a href="{{ route('frontend.product', [$row->id, $row->slug]) }}"><img
                                            src="{{ asset('media/' . $row->f_thumbnail) }}" alt="" /></a>
                                </div>
                                <h4 class="item-title"><a
                                        href="{{ route('frontend.product', [$row->id, $row->slug]) }}">{{ str_limit($row->title) }}</a>
                                </h4>
                                <div class="brand"><a
                                        href="{{ route('frontend.brand', [$row->brand_id, str_slug($row->brandname)]) }}">{{ str_limit($row->brandname) }}</a>
                                </div>
                                <div class="item-price-card">
                                    @if ($row->sale_price != '')
                                        @if ($gtext['currency_position'] == 'left')
                                            <div class="item-price">{{ $gtext['currency_icon'] }}{{ $row->sale_price }}
                                            </div>
                                        @else
                                            <div class="item-price">{{ $row->sale_price }}{{ $gtext['currency_icon'] }}
                                            </div>
                                        @endif
                                    @endif
                                    @if ($row->old_price != '')
                                        @if ($gtext['currency_position'] == 'left')
                                            <div class="old-item-price">
                                                {{ $gtext['currency_icon'] }}{{ $row->old_price }}</div>
                                        @else
                                            <div class="old-item-price">
                                                {{ $row->old_price }}{{ $gtext['currency_icon'] }}</div>
                                        @endif
                                    @endif
                                </div>
                                <div class="rating-wrap">
